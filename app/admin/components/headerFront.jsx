...
export default function HeaderFront({ ticketsSummary, activeFilter, onFilterChange }) {
  ...
  return (
    <header className="rounded-[28px] border border-[rgba(79,163,227,0.35)] bg-[rgba(18,21,36,0.88)] p-[1px] shadow-[0_25px_70px_rgba(11,14,31,0.6)] backdrop-blur">
      <div className="flex flex-col items-center rounded-[26px] border border-[rgba(79,163,227,0.25)] bg-[rgba(12,15,26,0.92)] px-10 py-12 text-center">
        <p className="text-lg font-semibold uppercase tracking-[0.35em] text-[#6E86E0]">Inspire IT Ticketing</p>

        <div ref={containerRef} className="relative mt-8 grid w-full gap-4 sm:grid-cols-3">
          {/* Tank transfer animation - drain downwards, fill upwards */}
          {isAnimating && (
            <>
              {/* Source button - color draining downwards */}
              <div
                className="absolute rounded-2xl pointer-events-none z-10 overflow-hidden"
                style={{
          ...
          <button
            ...
            className={`relative z-10 rounded-2xl border px-5 py-4 transition-[border,shadow] duration-300 ${
              activeFilter === 'all'
                ? 'border-[#6E63C6] bg-[rgba(110,99,198,0.25)] shadow-[0_0_25px_rgba(110,99,198,0.45)]'
                : 'border-[rgba(79,163,227,0.35)] bg-[rgba(27,30,56,0.3)] hover:border-[rgba(79,163,227,0.55)] hover:bg-[rgba(27,30,56,0.45)]'
            }`}
          >
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#7D8FEA]">Total Tickets</p>
            <p className="mt-2 text-3xl font-semibold text-[#F2F6FF]">{ticketsSummary.total}</p>
          </button>
          <button
          ...
          >
            <p className={`text-xs font-semibold uppercase tracking-[0.3em] transition-colors duration-300 ${activeFilter === 'unresolved' ? 'text-[#FF4C4C]' : 'text-[#7D8FEA]'}`}>Unresolved</p>
            <p className="mt-2 text-3xl font-semibold text-[#F2F6FF]">{ticketsSummary.unresolved}</p>
          </button>
          <button
          ...
          >
            <p className={`text-xs font-semibold uppercase tracking-[0.3em] transition-colors duration-300 ${activeFilter === 'resolved' ? 'text-[#4CFF7C]' : 'text-[#7D8FEA]'}`}>Resolved</p>
            <p className="mt-2 text-3xl font-semibold text-[#F2F6FF]">{ticketsSummary.resolved}</p>
          </button>
        </div>

        <p className="mt-6 text-xs text-[#8E97BA]">
          Navigate directly to <code className="rounded-md bg-black/40 px-2 py-1 text-xs">/admin</code> to access
